...
namespace Dolittle.Runtime.Events.Processing.Filters
{
    ...
    public class TypeFilterWithEventSourcePartitionValidator : ICanValidateFilterFor<TypeFilterWithEventSourcePartitionDefinition>
    {
        ...
        async Task ValidateBasedOffReFilteredStream(IFilterProcessor<TypeFilterWithEventSourcePartitionDefinition> filter, CancellationToken cancellationToken)
        {
            ...
            var artifactsFromStream = await _eventTypesFromStreams.FetchTypesInRange(filter.Definition.TargetStream, filter.SourceMicroservice, new StreamPositionRange(StreamPosition.Start, uint.MaxValue), cancellationToken).ConfigureAwait(false);

            var committedEvents = await _eventsFromStreams.FetchRange(filter.Definition.SourceStream, filter.SourceMicroservice, new StreamPositionRange(StreamPosition.Start, lastProcessedEventLogEvent.Value), cancellationToken).ConfigureAwait(false);
